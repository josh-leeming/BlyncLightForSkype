...
{
    public class SkypeMessageRouter : IMessageRouter<ISkypeMessage>
    {
        private static readonly object _lock = new object();
        private static readonly List<EventSubscription<ISkypeMessage>> _subscriptions = new List<EventSubscription<ISkypeMessage>>();
        ...
        public bool Publish(ISkypeMessage message)
        {
            var handled = false;
        ...
        public MessageSubscriptionToken Subscribe(Action<ISkypeMessage> handler)
        {
        ...
        /// <returns>A subscription token that can be used to modify this subscription</returns>
        public MessageSubscriptionToken Subscribe(Action<ISkypeMessage> handler, Predicate<ISkypeMessage> predicate, SubscriptionPriority priority = SubscriptionPriority.Normal)
        {
            EventSubscription<ISkypeMessage> es;

            lock (_lock)
            {
                es = new EventSubscription<ISkypeMessage> { Handler = handler, Predicate = predicate };
